...
def extract_labs(lab_config, time_window, AGG_FNC):
    labs = []
    for lab_name,obj in lab_config.items():
        print(lab_name)
        df = get_item_id(obj['idx'], time_window, AGG_FNC)
        if obj['post_fnc'] is not None:
            df = obj['post_fnc'](df)
        
        vmin = obj.get('vmin', None)
        vmax = obj.get('vmax', None)
        if vmin is not None:
            df = df.query(f"value>={vmin} and value<={vmax}")
...
def from_monitored_numeric(s3, flux_idx: List, config: Dict, time_window: int, debug: bool=False) -> Tuple[pd.DataFrame]: # , agg_fnc: List
    tables = get_tables(s3)
    
    vitals_list = {k: [] for k in config.keys()}
    flux = []
    if debug:
        tbls = tables.str.extract('(.*monitored_numeric.*)').dropna()[0].values[:10]
    else:
        tbls = tables.str.extract('(.*monitored_numeric.*)').dropna()[0].values
    for tbl_name in tqdm(tbls):
        ...
        for k,v in config.items():
            ...
            processed_ith.columns = processed_ith.columns.get_level_values(0)
            processed_ith = processed_ith.rename(columns={'value': k})
...
def get_septic_patients(sepsi_codes: Tuple[str]):
    dfs = []
    for i in range(1,7):
        diagnoses = wr.s3.read_parquet(f's3://icusics-db/diagnoses/diagnoses_h{i}.parquet')
        idx = diagnoses.referencecode.str.startswith(sepsi_codes)
        dfs.append(diagnoses.loc[idx])
    septic_pz = pd.concat(dfs, ignore_index=True)
    return septic_pz

def extract_urine_output(urine_codes: List[int], time_window: int) -> pd.DataFrame:
    urineOutput = []
    for i in tqdm(range(1,7)):
        labresults_numeric = wr.s3.read_parquet(f's3://icusics-db/observed_numeric/observed_numeric_h{i}.parquet')
        ith = labresults_numeric[labresults_numeric.a_variableid.isin(urine_codes)]
        urineOutput.append(ith)
        
    urineOutput = pd.concat(urineOutput)
    urineOutput = urineOutput.query(f"time<{time_window}")
    urineOutput = urineOutput.groupby('a_patientid').value.sum()
    urineOutput = urineOutput.rename('tot_urine').reset_index()
    urineOutput['hospital_id'] = urineOutput.a_patientid.apply(lambda x: int(str(x)[0]))
    urineOutput = urineOutput.set_index(['hospital_id', 'a_patientid'])
    return urineOutput